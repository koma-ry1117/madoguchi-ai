...
flowchart TB
    ...
    subgraph ExternalAPI["外部API"]
        Google["Google<br/>Gemini 2.5 Flash"]
        OpenAI["OpenAI<br/>Whisper"]
        GoogleTTS["Google Cloud TTS<br/>音声合成"]
...
| AI会話 | Google | Gemini 2.5 Flash |
| 音声認識 | OpenAI | Whisper |
| 音声合成 | Google Cloud TTS | 日本語音声生成（WaveNet） |
...
| Google AI SDK | Gemini連携 |
| OpenAI SDK | Whisper連携 |
| Resend | メール送信 |
...
| `/api/speech/synthesize` | POST | 音声合成（Google Cloud TTS） |
...
| 顧客情報アクセス制限 | システム管理者は各オペレーターの顧客情報（名前・電話番号・メールアドレス等）を閲覧不可。RLSにより技術的に制限 |
...
| API | 用途 | 料金目安 |
|-----|------|---------|
| Gemini 2.5 Flash | AI会話、物件情報構造化 | $0.15/$0.60/100万トークン（入力/出力） |
...
| 用途 | 料金目安 |
|------|---------|
| 音声合成（WaveNet/Neural2） | $16/100万文字 |
