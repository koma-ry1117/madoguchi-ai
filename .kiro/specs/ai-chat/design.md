...
stateDiagram-v2
    [*] --> Welcome: 画面アクセス
    Welcome --> Conversation: AI挨拶
    Conversation --> Conversation: 物件検索・提案
    Conversation --> ContactForm: 内見予約確定
    Conversation --> Goodbye: 終了ボタン
    ContactForm --> Complete: 連絡先入力完了
    Complete --> Welcome: 自動リセット
    Goodbye --> Welcome: 自動リセット
...
sequenceDiagram
    ...
    par メール送信
        API->>Email: 顧客確認メール
        API->>Email: オペレーター通知メール
    end

    API-->>UI: 完了
    UI->>UI: 完了画面表示

    Note over UI: 30秒後自動リセット
    UI->>UI: Welcome画面へ
```
...
interface MessageDisplay {
  ...
  audioState: 'preparing' | 'playing' | 'done' | 'skipped';
}
```
...
- **音声キャッシュ**: 頻出フレーズを事前キャッシュし、キャッシュヒット率40%以上を目標
